...
async function getSettings(supabase) {
  ...
  if (error || !Array.isArray(data)) return demoContentEnabled() ? DEFAULT_SETTINGS : {};
...
async function getProductList(supabase) {
  ...
  if (error || !Array.isArray(data)) return demoContentEnabled() ? PRODUCTS.map(publicProduct).filter((product) => product.enabled) : [];
