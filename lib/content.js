...
async function getSettings(supabase) {
  ...
  if (process.env.PUBLIC_SITE_MODE) settings.public_site_mode = process.env.PUBLIC_SITE_MODE;
  return settings;
}
...
async function updateSettingsHandler(req, res) {
  ...
  const auth = await requesterProfile(req, supabase);
  if (auth.error) return json(res, 401, { error: auth.error });
  if (!canAdmin(auth.profile)) return json(res, 403, { error: 'Only administrators can update site settings' });
  let body;
  try { body = await readJson(req); } catch { return json(res, 400, { error: 'Invalid JSON body' }); }
  const allowed = ['public_site_mode', 'company_name', 'company_email', 'company_phone', 'company_logo_url', 'primary_cta_text', 'primary_cta_url'];
  const rows = allowed.map((key) => ({ key, value: String(body[key] ?? DEFAULT_SETTINGS[key] ?? '') }));
